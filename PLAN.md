...
- [x] **MAPF algorithm zoo** (`mrn_coord.mapf`): 45+ algorithms faithfully
  ...
  `scripts/benchmark_gate.py` (full gate **86/86**). CBS family (CBSH, ECBS,
  ...
  **humanoid footstep planning (Hornung et al.) + multi-humanoid footstep
  MAPF**, and the low levels (space-time A\*, SIPP/SIPPS, Multi-Label A\*). Each
  documented algorithm-by-algorithm with its honest gated result in

...
- [x] **MAPF algorithm zoo** (`mrn_coord.mapf`): 45+ algorithms faithfully
  ...
  `scripts/benchmark_gate.py` (full gate **85/85**). CBS family (CBSH, ECBS,
  ...
  Passing, TPTS, online-LNS), execution (switchable-ADG, k-robust, TPG), and
  the low levels (space-time A\*, SIPP/SIPPS, Multi-Label A\*). Each documented
  algorithm-by-algorithm with its honest gated result in `docs/coordination.md`.

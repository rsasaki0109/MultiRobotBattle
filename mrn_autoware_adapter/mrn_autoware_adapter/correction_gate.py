"""Pure-Python safety gates for cooperative-pose-driven Autoware corrections.

This module is intentionally a verbatim copy of the gate semantics in
:mod:`mrn_nav2_adapter.correction_gate` — the safety rules ("don't ship a
cooperative pose that is stale, invalid, or jumps too far") are identical
for any cooperative-pose → external-localization adapter.

The duplication is deliberate: keeping each adapter's safety contract in
its own package keeps the cross-package dependency graph flat and lets
each adapter evolve its gates independently. A future v0.4 refactor may
move this to a shared ``mrn_core`` location once both adapters have
stabilized; until then, prefer copying to creating a foreign dependency.
"""
...
def evaluate(
    ...
    """Decide whether ``candidate`` should be applied as an Autoware correction.

#!/usr/bin/env python3
"""Headline demo — ORCA / BVC charge vs greedy on the chokepoint.

    python3 scripts/make_charge_gif.py --out docs/media/charge_layers.gif
"""
...
def _run_all(seed=4, max_ticks=700):
    runs = []
    for mode in CHARGE_HEADLINE_MODES:
        bots, cfg, title = charge_headline_duel(mode, seed=seed, n=10)
        res = simulate(bots, cfg, max_ticks=max_ticks)
        runs.append((cfg, res, CHARGE_HEADLINE_LABELS[mode]))
    return runs
...
def main():
    ...
    ap.add_argument("--out", default="docs/media/charge_layers.gif")
    ...
    fig.suptitle(
        "Headline demo — swap only the charge layer (red ORCA/BVC vs blue greedy)",
        color=render.INK, fontsize=12, fontweight="bold", y=0.98,
    )

    panels = []
    for ax, (cfg, res, title) in zip(axes.flat, runs):
        panels.append(ChargePanel(ax, cfg, res, title))
